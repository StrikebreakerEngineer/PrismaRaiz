...
if len(sys.argv) < 2:
    ...
    print("Example: python -m experimentos.crawler b", file=sys.stderr)
    sys.exit(1)

TARGET_LETTER = sys.argv[1].lower().strip()
if len(TARGET_LETTER) != 1:
    print("ERROR: Please provide a single letter (e.g., b, w, k)", file=sys.stderr)
    sys.exit(1)
...
TABLE_NAME = f"rae_letter_{TARGET_LETTER}"
METHOD = f"prefix_tree_{TARGET_LETTER}"
...
def main():
    ...
    print(f"\n================================================", flush=True)
    print(f"RAE PREFIX CRAWLER ('{TARGET_LETTER}' root -> master.db)", flush=True)
    print(f"================================================", flush=True)
    ...
    try:
        ...
        conn.close()
        print(f"\nCompleted. Total requests: {total_requests:,}, Unique IDs saved to {TABLE_NAME}: {total_new_ids:,}\n", flush=True)
